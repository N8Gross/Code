...
int main(int argc, char *argv[])
{
    ...
    int shm_fd1 = shm_open(array, O_CREAT | O_RDWR, 0666); //open the LowAlpha shared memory file descriptor for writing
    if(shm_fd1<1)
    {
      errnum = errno;
    ...
    if (argc != 3) {
    	printf("[LowAlpha]: Incorrect number of inputs! \n");
    	exit(-1);
    }
	...
	printf("LowAlpha[%d]: counted %d\n",getpid(),counter); //LowAlpha prints out its final result
